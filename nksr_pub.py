...
#         self.reconstructor = nksr.Reconstructor(
#             device=self.device,
#             config="ks"
#         )
...
#         self.get_logger().info("NKSR callable publisher ready")
    
#     @torch.no_grad()
#     def __call__(self, xyz, normals=None):
#         xyz = self._downsample(xyz)

#         if normals is None:
#             normals = self._fast_normals(xyz)

#         pts = torch.from_numpy(xyz).to(self.device, non_blocking=True)
#         nrm = torch.from_numpy(normals).to(self.device, non_blocking=True)

#         with torch.cuda.stream(self.stream):
#             field = self.reconstructor.reconstruct(
#                 pts, nrm, detail_level=0.15
#             )
...
#     #     """
#     #     Run NKSR reconstruction and publish mesh.
#     #     xyz: np.ndarray [N,3] in map frame
#     #     """

#     #     if xyz.shape[0] < 1000:
#     #         self.get_logger().warn("Too few points, skipping reconstruction")
#     #         return
...
#     #     # --- Torch tensors ---
#     #     pts = torch.from_numpy(np.asarray(pcd.points)).float().to(self.device)
#     #     nrm = torch.from_numpy(np.asarray(pcd.normals)).float().to(self.device)
...
#     #     self._publish_mesh(mesh)

#     def _publish_mesh(self, mesh):
#         marker = Marker()
